...
# EXACT LOCATION: Add this to your existing run_ultimate_seat_analysis() function
# Place it right after the "Generate general visualizations" section

def modify_your_existing_function():
    """
    In your existing run_ultimate_seat_analysis() function, 
    find this section and add the Kano analysis right after it:
    """
    
    # [Your existing code...]
    # Generate general visualizations
    training_visualizer.generate_general_visualizations(processed_ner_sentiment_df)

    # 🎯 ADD KANO ANALYSIS HERE - RIGHT AFTER THE LINE ABOVE
    # ================================================================
    logger.info("\n" + "="*50)
    logger.info("🎯 STARTING KANO ANALYSIS")
    logger.info("="*50)
    
    kano_results = integrate_kano_analysis(processed_ner_sentiment_df, output_base_dir)
            ...
            if entity in entity_data and entity_data[entity]:
                ...
                if len(entity_text.strip()) > 50:  # Minimum text length
                    try:
                        ...
                        logger.error(f"⚠️ Error generating {entity} word cloud: {e}")
        
        logger.info("✅ Entity-specific word clouds generated")
    ...
    def create_sentiment_wordclouds(self, text_data: List[str], stopwords: set):
        ...
        if positive_texts:
            ...
            try:
                ...
                logger.error(f"⚠️ Error generating positive sentiment word cloud: {e}")
        ...
        if negative_texts:
            ...
            try:
                ...
                logger.error(f"⚠️ Error generating negative sentiment word cloud: {e}")
        
        logger.info("✅ Sentiment-based word clouds generated")
...
class ComprehensiveTrainingVisualizer:
    ...
    def generate_ner_metrics_visualization(self, metrics_history: List[Dict]):
        ...
        self.plot_training_progress(metrics_history)
        self.plot_per_entity_progress(metrics_history)
        self.plot_final_results(metrics_history[-1] if metrics_history else {})
    ...
    def plot_per_entity_progress(self, metrics_history: List[Dict]):
        ...
        if rows == 1:
            axes = axes.reshape(1, -1) if n_entities > 1 else [axes]
        ...
        plt.suptitle('Per-Entity F1 Score Progress', fontsize=16, fontweight='bold')
        plt.tight_layout()
    ...
    def plot_final_results(self, final_metrics: Dict):
        """Plot final performance results"""
        if not final_metrics or 'per_entity' not in final_metrics:
            return

        per_entity_metrics = final_metrics.get('per_entity', {})
...
def create_ultimate_report(
    ...
    with open(report_path, "w", encoding='utf-8') as f:
        ...
        f.write("- `plots/training_progress.png` - Training progress with F1 scores\n")
        f.write("- `plots/per_entity_progress.png` - Individual entity progress over time\n")
        f.write("- `plots/ner_per_label_performance.png` - Final entity-wise performance\n")
        ...
        f.write("- `plots/component_sentiment.png` - Sentiment distribution by component\n")
        f.write("- `plots/component_frequency.png` - Component mention frequency\n")
        f.write("- `wordclouds/overall_wordcloud.png` - Overall text word cloud\n")
        f.write("- `wordclouds/positive_sentiment_wordcloud.png` - Positive sentiment words\n")
        f.write("- `wordclouds/negative_sentiment_wordcloud.png` - Negative sentiment words\n")
        f.write("- `wordclouds/[entity]_wordcloud.png` - Entity-specific word clouds\n")
...
def run_ultimate_seat_analysis(
    ...
    wordcloud_generator = WordCloudGenerator(output_base_dir)
    training_visualizer = ComprehensiveTrainingVisualizer(output_base_dir)
    ...
    if train_ner:
        ...
        if training_data and len(training_data) >= 5:
            ...
            if trained_ner_model_spacy:
                ...
                training_visualizer.generate_ner_metrics_visualization(ner_metrics_history)
    ...
    if lda_model and lda_dictionary and lda_corpus and not processed_ner_sentiment_df.empty:
        ...
        if kansei_results_per_review:
            ...
            training_visualizer.generate_kansei_visualizations(kansei_insights_data)
    ...
    # Prepare entity data for enhanced word clouds
    entity_data = {}
    if not processed_ner_sentiment_df.empty:
        for entity in STANDARDIZED_LABELS:
            entity_texts = processed_ner_sentiment_df[
                processed_ner_sentiment_df['Seat Component'] == entity
            ]['Cleaned Text'].tolist()
            if entity_texts:
                entity_data[entity] = entity_texts
    
    wordcloud_generator.generate_comprehensive_wordclouds(all_texts, entity_data)
